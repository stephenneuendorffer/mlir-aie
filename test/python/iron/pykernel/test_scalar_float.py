...
def test_nop(x:float):
    return x

def test_mul(x:float):
    return x*2.0

def test_div(x:float):
    return x/2.0

def test_add(x:float):
    return x+2.0

def test_sub(x:float):
    return x-2.0
...
def main():
    ...
    input0 = iron.zeros((args.num_elements,), dtype=np.float32, device="npu")
