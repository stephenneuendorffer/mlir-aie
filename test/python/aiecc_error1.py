...
# RUN: not %PYTHON %s --no-compile --no-link -nv 2>&1 | FileCheck %s

# CHECK: Dialect `linalg' not found for custom op 'linalg.fill'
...
module {
  ...
  %4 = aie.core(%12)  {
    ...
    linalg.fill ins(%f: f32) outs(%a : memref<5xf32>) 
